...
import 'package:untitled2/quiz/estados%20unidos/lessons_results_usa.dart';
import 'package:untitled2/quiz/estados%20unidos/loading_page_usa.dart';
import '../../funcionalidades/exitalert.dart';
import '../0firestore/firestore_services.dart';

class QuizUsa5 extends StatefulWidget {


  @override
  _QuizUsa5State createState() => _QuizUsa5State();
}
class _QuizUsa5State extends State<QuizUsa5> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "usaQuiz", "Qual o PIB dos Estados Unidos?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => LessonsResultsUsa()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == '21t400b') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "usaQuiz", "Qual o PIB dos Estados Unidos?");
      ...
      setState(() {
        _feedbackMessage = 'Resposta correta: 21 trilhôes e 400 bilhôes';
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Image.asset('images/circularusa.png', height: 150,),  // Adiciona a imagem acima do texto.
                ),
                Padding(
                  padding: const EdgeInsets.only(top: 40.0, bottom: 1.0),
                  child: Center(child: Text('Qual o PIB dos Estados Unidos?',
