...
/*
  Esta classe define a estrutura e funcionalidade do Quiz sobre o Brasil, todas as
  outras paginas de quiz são 'cópias' dessa porem com perguntas e respostas diferentes:
  1 A classe `QuizBrasil1` é um StatefulWidget, o que significa que ela tem um estado que pode mudar ao longo do tempo.
  O estado é representado pela classe `_QuizBrasil1State`. Ela mantém várias variáveis de estado que podem mudar durante
  a execução do aplicativo, como o ID do usuário, a resposta fornecida pelo usuário,
  a mensagem de feedback e outros elementos de interface.

  2 No método `initState`, a instância do FirebaseAuth é usada para obter o ID do usuário atual.
  Os métodos `_playCorrectAnswerAudio` e `_playWrongAnswerAudio` são usados para reproduzir diferentes
  sons de áudio dependendo se a resposta fornecida pelo usuário está correta ou incorreta. Há duas configurações
  de feedback para o usuario, mensagens(muito bem ou resposta errada) ou icones(certo ou errado), atualmente a pagina
  usa mensagens mas o codigo para uso de icones esta disponivel nela tambem

  3 A resposta do usuário é processada quando ele clica em uma letra. Cada letra clicada é adicionada à string da resposta do usuário.
  Quando o usuário clica no botão 'Verificar', a resposta do usuário é comparada com a resposta correta.
  Se a resposta estiver correta, a pontuação do usuário é incrementada, a questão é marcada como respondida
  e um som de resposta correta é reproduzido. Se a resposta estiver incorreta, um som de resposta incorreta é tocado.
  O layout da página é definido no método `build`. Ele inclui um botão 'Verificar' que, quando clicado,
  invoca o método `_onVerifyAnswer` para verificar a resposta do usuário.
  Além disso, a página apresenta uma lista de opções de respostas das quais o usuário pode escolher.
  O método `dispose` é usado para liberar os recursos (neste caso, o player de áudio)
  quando o widget é removido da árvore de widgets.
*/

class QuizBrasil1 extends StatefulWidget {


  @override
  _QuizBrasil1State createState() => _QuizBrasil1State();
}
class _QuizBrasil1State extends State<QuizBrasil1> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "brasilQuiz", "Qual a Capital do Brasil?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => QuizBrasil2()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == 'brasilia') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "brasilQuiz", "Qual a Capital do Brasil?");
      ...
      setState(() {
        _feedbackMessage = 'Resposta correta: Brasilia';
