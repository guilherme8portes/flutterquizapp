...
import 'package:untitled2/quiz/japao/quiz_japao_4.dart';
import '../../funcionalidades/exitalert.dart';
import '../0firestore/firestore_services.dart';

class QuizJapao3 extends StatefulWidget {


  @override
  _QuizJapao3State createState() => _QuizJapao3State();
}
class _QuizJapao3State extends State<QuizJapao3> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "japaoQuiz", "Qual a empresa mais valiosa do Japão?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => QuizJapao4()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == 'toyota') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "japaoQuiz", "Qual a empresa mais valiosa do Japão?");
      ...
      setState(() {
        _feedbackMessage = 'Resposta correta: Toyota';
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Image.asset('images/circularjapao.png', height: 150,),  // Adiciona a imagem acima do texto.
                ),
                Padding(
                  padding: const EdgeInsets.only(top: 40.0, bottom: 1.0),
                  child: Center(child: Text('Qual a empresa mais valiosa do Japão?',
