...
import 'package:untitled2/quiz/japao/quiz_japao_5.dart';
import '../../funcionalidades/exitalert.dart';
import '../0firestore/firestore_services.dart';

class QuizJapao4 extends StatefulWidget {


  @override
  _QuizJapao4State createState() => _QuizJapao4State();
}
class _QuizJapao4State extends State<QuizJapao4> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "japaoQuiz", "Qual a população do Japão?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => QuizJapao5()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == '126 milhões') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "japaoQuiz", "Qual a população do Japão?");
      ...
      setState(() {
        _feedbackMessage = 'Resposta correta: 126 milhões';
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Center(child: Text('Qual a população do Japão?',
