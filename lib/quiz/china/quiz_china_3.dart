...
import 'package:untitled2/quiz/china/quiz_china_4.dart';
import '../../funcionalidades/exitalert.dart';
import '../0firestore/firestore_services.dart';

class QuizChina3 extends StatefulWidget {


  @override
  _QuizChina3State createState() => _QuizChina3State();
}
class _QuizChina3State extends State<QuizChina3> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "chinaQuiz", "Qual a empresa mais valiosa da China?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => QuizChina4()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == 'tencent') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "chinaQuiz", "Qual a empresa mais valiosa da China?");
      ...
      setState(() {
        _feedbackMessage = 'Resposta correta: Tencent';
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Image.asset('images/circularchina.png', height: 150,),  // Adiciona a imagem acima do texto.
                ),
                Padding(
                  padding: const EdgeInsets.only(top: 40.0, bottom: 1.0),
                  child: Center(child: Text('Qual a empresa mais valiosa da China?',
