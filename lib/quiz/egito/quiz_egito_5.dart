...
import 'package:untitled2/quiz/egito/lessons_results_egito.dart';
import '../../funcionalidades/exitalert.dart';
import '../0firestore/firestore_services.dart';

class QuizEgito5 extends StatefulWidget {


  @override
  _QuizEgito5State createState() => _QuizEgito5State();
}
class _QuizEgito5State extends State<QuizEgito5> {
  ...
  void _onVerifyAnswer() async {
    ...
    if (await firestoreService.isQuestionAnswered(userId, "egitoQuiz", "Qual o PIB do Egito?")) {
      setState(() {
        _feedbackMessage = 'Você já respondeu essa pergunta corretamente antes!';
        Future.delayed(const Duration(seconds: 3), () async{
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => LessonsResultsEGITO()),
          );
        }
        );
      });
    ...
    if (_userAnswer.trim().toLowerCase() == '360b') {
      setState(() {
      ...
      firestoreService.markQuestionAsAnswered(userId, "egitoQuiz", "Qual o PIB do Egito?");
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Center(child: Text('Qual O PIB do Egito?',
